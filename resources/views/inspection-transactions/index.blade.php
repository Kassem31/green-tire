    ...
    {{-- <div class="middle-content container-xxl p-0"> --}}
        <div class="row layout-top-spacing">
            <div class="page-title">
                <h3>{{ __('navigation.inspection_transactions') }}</h3>
            </div>

            <div class="d-flex flex-wrap gap-2">
                <a href="{{ route('inspection-transactions.create') }}" class="btn btn-primary me-2">{{ __('buttons.create_inspection_transaction') }}</a>
                    ...
                    <form method="GET" action="{{ route('inspection-transactions.index') }}" class="mb-4">
                        <div class="row align-items-end mb-2">
                            <div class="col-md-2 col-12 filter-column mb-2">
                                <label for="filter-barcode" class="form-label">{{ __('forms.inspection_barcode') }}</label>
                                <input type="text" id="filter-barcode" name="barcode" class="form-control"
                                    placeholder="{{ __('forms.search_placeholder') }}" value="{{ request('barcode') }}">
                            </div>
                            <div class="col-md-2 col-12 filter-column mb-2">
                            ...
                            <div class="col-md-2 col-12 filter-column mb-2">
                                ...
                                <select id="filter-decision" name="decision" class="form-control">
                                    ...
                                    <option value="Repair" {{ request('decision') == 'Repair' ? 'selected' : '' }}>{{ __('forms.repair') }}</option>
                                    <option value="Scrap" {{ request('decision') == 'Scrap' ? 'selected' : '' }}>{{ __('forms.scrap') }}</option>
                                </select>
                            </div>
                            <div class="col-md-2 col-12 filter-column mb-2">
                            ...
                            <div class="col-md-2 col-12 filter-column mb-2">
                                ...
                                <input type="text" id="filter-machine" name="machine" class="form-control"
                                    placeholder="{{ __('forms.filter_by_machine') }}" value="{{ request('machine') }}">
                            </div>
                            <div class="col-md-2 col-12 filter-column mb-2">
                                ...
                                <select id="filter-status" name="status" class="form-control">
                                    ...
                                    <option value="repaired" {{ request('status') == 'repaired' ? 'selected' : '' }}>{{ __('forms.repaired') }}</option>
                                    <option value="pending" {{ request('status') == 'pending' ? 'selected' : '' }}>{{ __('forms.pending_repair') }}</option>
                                    <option value="scrap" {{ request('status') == 'scrap' ? 'selected' : '' }}>{{ __('forms.scrapped') }}</option>
                                </select>
                            </div>
                    ...
                    <table id="zero-config" class="table dt-table-hover" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td data-th="{{ __('forms.tire_type') }}">{{ $transaction->tireType->name_en }}</td>
                                <td data-th="{{ __('forms.decision') }}">{{ $transaction->decision }}</td>
                                <td data-th="{{ __('forms.building_date') }}">{{ $transaction->building_date ? $transaction->building_date->format('Y-m-d') :
                                    __('forms.not_available') }}</td>
                                <td data-th="{{ __('forms.machine') }}">{{ $transaction->machine ?? __('forms.not_available') }}</td>
                                ...
                                <td class="text-center" data-th="{{ __('forms.actions') }}">
                                    <div class="d-flex flex-wrap justify-content-center button-group gap-1">
                                        ...
                                        <a href="{{ route('repair-transactions.create', ['inspection_id' => $transaction->id]) }}"
                                            ...
                                            data-bs-placement="top" title="{{ __('forms.add_repair_steps') }}">
