...
class InspectionTransactionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($existingTransaction) {
            return redirect()->back()
                ->withInput()
                ->withErrors(['barcode' => __('messages.barcode_already_exists')]);
        }
        ...
        try {
            ...
            if (count($results) === 0 || !$results[0]->TRC_BARCODE) {
                return redirect()->back()
                    ->withInput()
        ...
        DB::connection('SP_Connection')->statement('EXEC [dbo].SET_BLOCK_GT_BAPTIZE ?, 1', [
            $validated['barcode'],
        ]);
        ...
        return redirect()->route('inspection-transactions.index')
            ->with('success', __('messages.inspection_transaction_created_successfully'));
    }
    ...
    public function update(Request $request, InspectionTransaction $inspectionTransaction)
    {
        ...
        if ($existingTransaction) {
            return redirect()->back()
                ->withInput()
                ->withErrors(['barcode' => __('messages.barcode_used_by_another')]);
        }
        ...
        return redirect()->route('inspection-transactions.index')
            ->with('success', __('messages.inspection_transaction_updated_successfully'));
    }
    ...
    public function destroy(InspectionTransaction $inspectionTransaction)
    {
        ...
        return redirect()->route('inspection-transactions.index')
            ->with('success', __('messages.inspection_transaction_deleted_successfully'));
    }
}
